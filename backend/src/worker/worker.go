...
func StartWorker() {
	...
	for {
		...
		go func(j IngestJob) {
			// A single bad page must never kill the API + worker loop.
			defer func() {
				if r := recover(); r != nil {
					log.Printf("Job panicked for Bookmark ID %d: %v\n", j.BookmarkID, r)
					_, _ = db.Pool.Exec(context.Background(),
						"UPDATE bookmarks SET summary = $1 WHERE id = $2",
						"Processing failed unexpectedly. You can retry or delete this bookmark.", j.BookmarkID,
					)
				}
			}()
...
// whitespace or end of text. RE2 (Go regexp) has no lookbehind, so this
// is a tiny hand-rolled splitter that keeps the punctuation attached.
func splitSentences(s string) []string {
	...
	for i := 0; i < len(s); i++ {
		...
		if j > i+1 || i == len(s)-1 {
			if piece := strings.TrimSpace(s[start:j]); piece != "" {
...
Summary:`, title, truncateString(text, 3000))
	...
	// NOTE: Go's RE2 has no lookbehind, so split manually on
	...
	sentences := splitSentences(cleanSnippet)

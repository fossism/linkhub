...
import { 
  Folder, Plus, Search, LogOut, Star, Link2, 
...
interface DecryptedViewerState {
  bookmark: Bookmark;
  assetType: string;
...
export const Dashboard: React.FC<DashboardProps> = ({ token, encryptionKey, user, onLogout, apiUrl }) => {
  ...
  const [screenshotCache, setScreenshotCache] = useState<Record<number, string>>({});
  const [decryptedViewer, setDecryptedViewer] = useState<DecryptedViewerState | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (selectedTag !== null) params.append('tagId', selectedTag.toString());
    ...
    fetchData();
  }, [selectedCategory, selectedTag, showFavoritesOnly, refreshTrigger, token, apiUrl, semanticSearch]);
  ...
  const handleViewAsset = async (bookmark: Bookmark, type: string) => {
    ...
    try {
      ...
      setDecryptedViewer(prev => {
        if (!prev) return null;
        return { ...prev, objectUrl: url, loading: false };
      });
    } catch (err) {
  ...
  const Thumbnail: React.FC<{ bookmark: Bookmark }> = ({ bookmark }) => {
          ...
          <div>
            ...
            <nav className="space-y-1">
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition font-medium ${selectedCategory === null ? 'bg-white/5 text-accentBlue shadow-inner' : 'text-slateText hover:bg-white/5 hover:text-white'}`}
              >
        ...
        <header className="glass-panel border-b border-white/5 p-4 sticky top-0 z-20 flex items-center justify-between gap-4">
          <form onSubmit={triggerSearch} className="flex-1 max-w-lg flex items-center gap-2">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold border transition ${semanticSearch ? 'bg-cyan-955/30 border-cyan-500/50 text-accentBlue shadow-neonCyan' : 'bg-slate-900/40 border-white/5 text-slateText hover:border-white/10 hover:text-white'}`}
        ...
        <div className="fixed inset-0 bg-darkBg/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="glass-panel w-full max-w-lg rounded-2xl shadow-glass border border-white/5 overflow-hidden animate-slide-in">
            ...
            <form onSubmit={handleAddLink} className="p-6 space-y-4">
              ...
              <div className="p-3 bg-cyan-955/20 border border-cyan-800/30 rounded-xl flex gap-3 text-xs text-slateText">
          ...
          <div className="glass-panel w-full max-w-5xl h-[85vh] rounded-2xl border border-white/10 flex flex-col overflow-hidden animate-slide-in shadow-2xl">
            ...
            <div className="p-4 border-b border-white/5 flex items-center justify-between bg-slate-950/40">
              ...
              <div className="flex items-center gap-1.5 bg-slate-900/60 p-1 rounded-xl border border-white/5">
                {decryptedViewer.bookmark.assets && decryptedViewer.bookmark.assets.map(asset => (
                  <button
                    key={asset}
            ...
            <div className="flex-1 overflow-auto bg-slate-955/20 relative">
              {decryptedViewer.loading ? (
                ...
                <div className="w-full h-full p-6 flex justify-center">
                  ...
                  {decryptedViewer.assetType === 'screenshot' && decryptedViewer.objectUrl && (
                    <img 
                  ...
                  {decryptedViewer.assetType === 'pdf' && decryptedViewer.objectUrl && (
                  ...
                  {decryptedViewer.assetType === 'html_dump' && decryptedViewer.objectUrl && (

...
export const Auth: React.FC<AuthProps> = ({ onAuthSuccess, apiUrl }) => {
  ...
  // Read a fetch Response as JSON, but translate empty / non-JSON bodies
  // (e.g. backend down, proxy 500) into a human-readable error instead of
  // the cryptic "JSON.parse: unexpected end of data".
  const readJsonBody = async (res: Response, action: string): Promise<any> => {
    const text = await res.text();
    if (!text) {
      throw new Error(
        `Cannot reach the LinkHub server while ${action}. Is the backend running on port 5000?`
      );
    }
    try {
      ...
      throw new Error(
        `Unexpected server response while ${action}. Is the backend running?`
      );
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        const { salt } = await readJsonBody(saltRes, 'fetching sign-in parameters');
        ...
        const data = await readJsonBody(loginRes, 'signing in');
        if (!loginRes.ok) {
        ...
        const data = await readJsonBody(regRes, 'creating your account');
        if (!regRes.ok) {
      ...
      console.error(err);
      // fetch() itself throws a bare TypeError when the server is unreachable.
      const message =
        err instanceof TypeError
          ? 'Cannot reach the LinkHub server. Is the backend running on port 5000?'

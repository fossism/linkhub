...
export const App = () => {
  const [token, setToken] = useState(() => localStorage.getItem('linkhub_token') || null);
  const [user, setUser] = useState(() => {
    const saved = localStorage.getItem('linkhub_user');
    return saved ? JSON.parse(saved) : null;
  });
  const [encryptionKey, setEncryptionKey] = useState(() => sessionStorage.getItem('linkhub_ek') || null);
  ...
  const handleAuthSuccess = (newToken, newEncryptionKey, userData) => {
